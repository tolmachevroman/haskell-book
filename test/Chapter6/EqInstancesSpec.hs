...
spec :: Spec
spec =
  describe "Instances exercises" $ do
    it "TisAnInteger is an instance of Eq and Show" $ TisAn 5 `shouldBe` TisAn 5
    it "TwoIntegers is an instance of Eq and Show" $
      Two 5 10 `shouldBe` Two 5 10
    ...
    it "EitherOr is an instance of Eq and Show" $ do
      (Hello 'b' :: EitherOr Char Int) `shouldBe`
        (Hello 'b' :: EitherOr Char Int)
      (Goodbye "hello" :: EitherOr Int String) `shouldBe`
        (Goodbye "hello" :: EitherOr Int String)
    ...
    it "Papu is an instance of Eq, Ord and Show" $ do
      ...
      compare (Papu (Rocks "ko") (Yeah True)) (Papu (Rocks "ko") (Yeah False)) `shouldBe`
        GT
